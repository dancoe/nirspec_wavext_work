...
OUT_DIR = f'{BASE}/nirspec_wavext_work/reports/v9/ifu_v9/plots/parlanti'
...
def plot_v9_coefficients(mode, out_filename, add_faint_ref=False):
    """Plot our v9 coefficients (2 panels: G140M, G235M) with all 3 vars."""
    ...
    for i, (g, title) in enumerate(zip(gratings, titles)):
        ...
        ax.set_title(f'{mode.upper()} {title} coeffs (This work v9)', fontsize=14)
        ax.grid(False) 
        ...
        if i == 0:
            ...
            if add_faint_ref:
                legend_elements.extend([
                    Line2D([0], [0], color='black', lw=1.5, ls='-', label='This work (v9)'),
...
def plot_single_coefficient_comparison(mode, var_name):
    """Plot a single coefficient comparison (This work v9 vs Parlanti+25) across 2 panels."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=True)
    plt.subplots_adjust(wspace=0.1)
    ...
    for i, (g, title) in enumerate(zip(gratings, titles)):
        ...
        if os.path.exists(path):
            with fits.open(path) as h:
                ...
                val_v9 = v8_data[var_name]
            
            ax.plot(wl, val_v9, color=color, lw=2.0) 

        ax.set_yscale('log')
        ax.set_ylim(1e-4, 10.0)
        ax.set_xlim(XRANGES[g])
        ax.set_xlabel(r'Wavelength [$\mu$m]', fontsize=12)
        ax.set_title(f'{mode.upper()} {title} {label} (This work v9)', fontsize=14)
        ax.grid(False) 
        
        if i == 0:
            ax.set_ylabel(f'Value ({label})', fontsize=12)
            legend_elements = [
                Line2D([0], [0], color=color, lw=2.0, ls='-', label=f'This work (v9)'),
                Line2D([0], [0], color=color, lw=1.0, ls='--', alpha=0.45, label='Parlanti+25'),
            ]
            ax.legend(handles=legend_elements, loc='lower left', fontsize=10)

    out_filename = f'parlanti_coefficients_v9_{mode}_{var_name}_comparison.png'
    out_path = os.path.join(OUT_DIR, out_filename)
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {out_path}")
...
if __name__ == '__main__':
    ...
    plot_v9_coefficients('ifu', 'parlanti_coefficients_v9_ifu.png', add_faint_ref=False)
    plot_v9_coefficients('ifu', 'parlanti_coefficients_v9_ifu_with_ref.png', add_faint_ref=True)
