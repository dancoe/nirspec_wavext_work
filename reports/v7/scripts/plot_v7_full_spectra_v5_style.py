...
OUTPUT_DIR  = f'{BASE}/nirspec_wavext_work/reports/v7/fs_v7/plots'
...
def load_coeffs(grating):
    ...
    with fits.open(fits_path) as h:
        ...
        # Handle possible NaNs in coeffs
...
def plot_full_spectra(pid, src):
    ...
    fig.suptitle(f'{name} — PID {pid} — FS wavext v7', fontsize=16)
    ...
    for f in files:
        ...
        # Skip some intermediate products if they contain stage numbers we don't want
        ...
        # Darker for NRS1, lighter for NRS2 (as per v5 style)
        alpha = 0.4 if detector == 'NRS2' else 0.7
        lw = 0.8
        
        # Determine if this is an extension vs nominal
        # In v7 log, NRS2 files in nrs2_spec2_cal/ are the ones we want to correct.
        # But wait, stage 3 files might also be here.
        # Let's just plot all, and highlight the corrected NRS2 region.
        ...
        # Recalibration (Extension logic)
        path_lower = f.lower()
        if detector == 'NRS2' and grating in ['G140M', 'G235M'] and 'nrs2_spec2_cal' in path_lower and f_ref:
            ki, ai, bi = coeffs[grating]
            if ki is not None:
                # Same cutoff logic as v5? 
                # Actually, in v7 we fit the whole range, but let's show the ghost-dominant part clearly.
    ...
    # These are usually the files in the base directory or stage3_cal
    ...
    for pat in truth_patterns:
        for d in src['dirs']:
            ...
            for tf in truth_files:
                ...
                if wt is not None and det == 'NRS1': # Only plot NRS1 as "truth" baseline
                    ax.plot(wt, ft, color='black', lw=1, alpha=0.6, ls='--', zorder=4)

    # CALSPEC Model & Ghosts (Goldenrod)
