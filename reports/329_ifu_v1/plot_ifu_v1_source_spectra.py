...
COEFFS_DIR  = f'{BASE}/nirspec_wavext_work/plots/Parlanti/cal/ifu_v1'
OUTPUT_DIR  = f'{BASE}/nirspec_wavext_work/reports/329_ifu_v1'
...
SOURCES = {
    '1538': {
        ...
        'g395m_truth': f'{FS_DIR}/PID1538_P330E/jw01538-o160_t002-s000000001_nirspec_f290lp-g395m-s1600a1-sub2048_x1d.fits',
    },
    '1537': {
        'name':     'G191-B2B',
        'calspec':  'g191b2b_mod_012.fits',
        'ifu_dir':  f'{IFU_DIR}/PID1537_G191-B2B',
        'g395m_truth': f'{FS_DIR}/PID1537_G191-B2B/jw01537-o007_t001-s000000001_nirspec_f290lp-g395m-s1600a1-sub2048_x1d.fits',
    },
    '1536': {
        'name':     'J1743045',
        'calspec':  '1743045_mod_007.fits',
        'ifu_dir':  f'{IFU_DIR}/PID1536_J1743045',
        'g395m_truth': f'{FS_DIR}/PID1536_J1743045/jw01536-o002_t004-s000000001_nirspec_f290lp-g395m-s1600a1-sub2048_x1d.fits',
    },
}
...
def plot_source_spectra(sid, src):
    ...
    fig.suptitle(f'IFU v1 Calibration (329) — {name} Spectra Comparison', fontsize=14)
    ...
    for i, grating in enumerate(['G140M', 'G235M']):
        ...
        # Load Coefficients
        csv_path = f'{COEFFS_DIR}/coeffs_ifu_v1_{grating}.csv'
        ...
        # Load Data
        if grating == 'G140M':
            # Observed: IFU G140M stage3_ext
            x1d_ext_path  = os.path.join(src['ifu_dir'], 'stage3_ext/f100lp_g140m-f100lp_x1d.fits')
            x1d_truth_path = os.path.join(src['ifu_dir'], 'stage3/f170lp_g235m-f170lp_x1d.fits')
            nrs2_lo = 1.87
        else: # G235M
            x1d_ext_path  = os.path.join(src['ifu_dir'], 'stage3_ext/f170lp_g235m-f170lp_x1d.fits')
            x1d_truth_path = src['g395m_truth']
            nrs2_lo = 3.15
        
        # Load NRS2 pixels only from extended
        w_obs_all, f_obs_all = load_spec(x1d_ext_path)
        w_obs = w_obs_all[w_obs_all >= nrs2_lo]
        f_obs = f_obs_all[w_obs_all >= nrs2_lo]
        
        w_truth, f_truth = load_spec(x1d_truth_path)
        ...
        # Plotting
        ax.plot(w_obs,  f_obs,  'k', lw=1.0, alpha=0.3, label=f'Observed {grating} (Extended NRS2)')
        ...
        ax.plot(w_obs,  f_corr,  'r', lw=1.5, label=f'Recalibrated {grating} (Parlanti Corr)')
    ...
    plt.tight_layout()
    outpath = f'{OUTPUT_DIR}/ifu_v1_spectra_{name}.png'
